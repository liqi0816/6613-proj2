...
def train_EWC(optimizer, ewc, criterion, mb_size, x, y, t,
              train_ep, preproc=None, use_cuda=True, mask=None):
    """
    Train a Pytorch ewc model from pre-loaded tensors.

        Args:
            optimizer (object): the pytorch optimizer.
            ewc (object): the pytorch ewc model to train.
    ...
    for ep in range(train_ep):
        ...
        for it in range(it_x_ep):
            ...
            logits = ewc.model(x_mb)

            _, pred_label = torch.max(logits, 1)
            correct_cnt += (pred_label == y_mb).sum()
            loss = ewc._compute_consolidation_loss(ewc.weight) + ewc.crit(logits, y_mb)
            ave_loss += loss.item()
            loss.backward()
            optimizer.step()
...
def train_net(optimizer, model, criterion, mb_size, x, y, t,
    ...
    Train a Pytorch model from pre-loaded tensors.
        ...
        Args:
            optimizer (object): the pytorch optimizer.
            m:odel (object): the pytorch model to train.
            criterion (func): loss function.
            mb_size (int): mini-batch size.
            x (tensor): train data.
            y (tensor): train labels.
